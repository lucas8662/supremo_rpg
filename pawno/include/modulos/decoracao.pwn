...
Comando(carregartema)
{
	...
	if(!isnull(TemaCarregado))
		return SendClientMessage(playerid, COLOR_ERRO, fmat_return("|ERROR| O servidor já está carregado com o tema %s, use /descarregartema.", TemaCarregado));
	...
	ShowPlayerDialog(playerid, DIALOG_CARREGARTEMA, DIALOG_STYLE_LIST, "» {1E90FF}Temas pre-definidos »{FFFFFF} Carregar", gs_Buffer, "Carregar", "Fechar");
	return true;
}
